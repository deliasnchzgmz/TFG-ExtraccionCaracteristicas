...
def lines_location (bin_img, counter):
    line_pos = []
    delete_lines = []
    ##comprueba cuantos pixeles negros hay en cada fila
  ...
  if (len(line_pos)%5 == 0):
    ...
    for group in range(0,num_staffs):
      ...
      for space in range(1,len(line_pos)):
        diff = diff + (line_pos[space]-line_pos[space-1]) 
        #media de los espacios para colocar una línea al mismo espacio por debajo del pentagrama

      ##líneas del pentagrama
      st.append([int(line_pos[0+5*group]-diff/4), line_pos[0+5*group], line_pos[1+5*group], 
                    line_pos[2+5*group], line_pos[3+5*group], line_pos[4+5*group]])
...
def detect_ei(bin_img):
    ...
    horizontalStructureD = cv2.getStructuringElement(cv2.MORPH_RECT, (30, 1))
...
def get_rectangles_gen(mscore_img, template,  name, threshold):
    h = template.shape[0]
    w = template.shape[1]
    result = cv2.matchTemplate(mscore_img, template, cv2.TM_CCOEFF_NORMED)
...
def draw_rectangles(mscore_img, l):
    ...
    for i in range(len(l)):
        ...
        if len(l[i])>2:
            cv2.putText(mscore_img, n[l[i][2]], (l[i][0][0], l[i][0][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        else: cv2.putText(mscore_img, l[i][1], (l[i][0][0], l[i][0][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

def order_lists(clef, clef_out, c, c_out, crotchet, crotchet_out, minim, minim_out, quaver, quaver_out, crotchetR, minimR, minimR_out, quaverR, quaverR_out, sharp_list, sharp_out, flat_list, flat_out, natural_list, natural_out):
    full_list = []
    ...
    for i in range(len(sharp_list)):
        full_list.append([sharp_list[i], sharp_out[0]])
    for i in range(len(flat_list)):
        full_list.append([flat_list[i], flat_out[0]])
    for i in range(len(natural_list)):
        full_list.append([natural_list[i], natural_out[0]])
    ...
    if(len(loc)>0):
        for c in reversed(range(0,len(crotchet))):
            for l in range(len(loc)):
                if(loc[l] and (crotchet[c][0]==loc[l] or round(crotchet[c][0]+crotchet[c][2]*1.5)==loc[l] or 
                    (abs(crotchet[c][0]-loc[c]<12) ) )):
                   delete.append(c)  
